...
from tests.conftest import response_to_json, setup_token, setup_second_token
from tests.helpers import constants
from tests.helpers.database.query_hermes import QueryHermes
from tests.helpers.test_context import TestContext
from tests.helpers.test_data_utils import TestDataUtils
from tests.helpers.test_helpers import TestData, Merchant
...
def verify_loyalty_card_into_database(journey_type, merchant):
    ...
    assert scheme_account.id == TestContext.current_scheme_account_id \
        ...
        f"Scheme Account is not Active and the status is '{scheme_account.status}'"
...
def verify_invalid_request(merchant, invalid_request, status_code):
    ...
    logging.info("The response of Invalid json Journey (POST) for Add field:\n \n"
                 + Endpoint.BASE_URL + api.ENDPOINT_MEMBERSHIP_CARDS + "\n\n"
                 + json.dumps(response_json, indent=4))
...
def verify_invalid_token_bearer_prefix_for_membership_card(merchant):
    response = MembershipCards.add_field_only_card(TestDataUtils.TEST_DATA.invalid_token.get(constants.INVALID_TOKEN), merchant)
