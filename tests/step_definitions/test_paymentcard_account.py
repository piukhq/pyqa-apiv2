...
def update_payment_account(update_field, payment_card_provider):
        response = PaymentCards.update_payment_card(TestContext.token, payment_card_provider, update_field, TestContext.current_payment_card_id)
        TestContext.response_status_code = response.status_code
...
def verify_update_field_payment_account(payment_card_provider,update_field):
    ...
    if update_field == "expiry_month":
        ...
        assert TestContext.name_on_payment_card == TestContext.response_json.get("name_on_card"), "Name on card not updating"
    elif update_field == "card_nickname":
        assert TestContext.card_nickname == TestContext.response_json.get("card_nickname"), "Nick name on card not updating"
    elif update_field == "issuer":
        assert  PaymentCardTestData.get_data(payment_card_provider).get(constants.ISSUER_UPDATED) ==\
        ...
        assert TestContext.name_on_payment_card == TestContext.response_json.get(
            "name_on_card") and TestContext.card_nickname == TestContext.response_json.get(
            "card_nickname") and PaymentCardTestData.get_data(payment_card_provider).get(
            constants.ISSUER) == TestContext.response_json.get(
            "issuer") and TestContext.expiry_month == TestContext.response_json.get(
            "expiry_month") and TestContext.expiry_year == TestContext.response_json.get("expiry_year")
