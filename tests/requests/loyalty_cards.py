...
class MembershipCards(Endpoint):
    ...
    @staticmethod
    def get_payment_account_channel_links(token):
        url = Endpoint.BASE_URL + api.ENDPOINT_WALLET_PAYMENT_ACCOUNT_CHANNEL_LINKS
        header = Endpoint.request_header(token, "2.0")
        response = Endpoint.call(url, header, "GET")
        return response
    ...
    @staticmethod
    def add_and_auth_field_with_unauthorised_json(token, merchant, request_payload):
    ...
    def get_add_url(scheme_account_id=None):
        """Return URL for loyalty_cards and
    ...
    @staticmethod
    def add_and_authorise_transactions_card(token, merchant, card):
        url = MembershipCards.get_add_and_authorise_url()
        header = Endpoint.request_header(token)
        payload = Merchant.get_merchant(merchant).add_and_authorise_transactions_card_payload(card)
        return Endpoint.call(url, header, "POST", payload)
