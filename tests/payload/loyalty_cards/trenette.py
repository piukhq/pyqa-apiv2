...
class TrenetteCard:
    ...
    def add_field_only_membership_card_payload(invalid_data=None):
        if invalid_data:
            ...
            TestContext.card_number = TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.CARD_NUM)
            payload = {
                "account": {
                    "add_fields": {
                        "credentials": [
                ...
                "loyalty_plan_id": TestDataUtils.TEST_DATA.membership_plan_id.get("trenette"),
            }
        logging.info(
    ...
    def add_field_only_transactions_membership_card_payload(invalid_data=None):
        if invalid_data:
            ...
            TestContext.card_number = TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.TRANSACTIONS_CARD)
            payload = {
                "account": {
                    "add_fields": {
                        "credentials": [
            ...
            "account": {
                "add_fields": {
                    "credentials": [
                        {
                            "credential_slug": "'card_number'",
                            "value": TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.CARD_NUM),
                        }
                    ]
                }
            },
    ...
    def add_and_authorise_membership_card_payload(invalid_request=None):
        TestContext.card_number = TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.CARD_NUM)
        TestContext.email = TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.EMAIL)
        if invalid_request:
            payload = {}
        else:
            payload = {
    ...
    def add_and_authorise_transactions_card_payload():
        TestContext.card_number = TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.TRANSACTIONS_CARD)
        TestContext.email = TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.TRANSACTIONS_EMAIL)
        payload = {
            "account": {
                "add_fields": {
                    "credentials": [
    ...
    def add_and_auth_field_only_membership_card_with_unauthorised_json():
        TestContext.card_number = TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.CARD_NUM)
        TestContext.email = TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.UNAUTHORISED_EMAIL)
        payload = {
            "account": {
                "add_fields": {
                    "credentials": [
                ...
                "account": {
                    "authorise_fields": {
                        "credentials": [
                            {
                                "credential_slug": "'email'",
                                "value": TestDataUtils.TEST_DATA.trenette_membership_card.get(
    ...
    def add_and_authorise_with_different_auth_field():
            ...
            "account": {
                ...
                "authorise_fields": {
                    "credentials": [
                        {
                            "credential_slug": "email",
                            "value": TestDataUtils.TEST_DATA.trenette_membership_card.get(constants.INVALID_EMAIL),
                        },
                    ]
                },
            },
        ...
        logging.info(
            "The Request for Trenette Add_and_Auth journey with different auth value:\n"
            + Endpoint.BASE_URL
            + api.ENDPOINT_MEMBERSHIP_CARDS_ADD_AND_AUTHORISE
            + "\n\n"
            + json.dumps(payload, indent=4)
