...
so that I am aware and have visibility of the vouchers that have been concluded with my loyalty card
  ...
  Scenario Outline: Get Loyalty card vouchers
    ...
    And I perform GET request to view loyalty card vouchers for "<merchant>" with "<state>", "<progress_display_text>", "<current_value>", "<target_value>" and "<suffix>"
    Then I see a <status_code_returned>
    And I perform DELETE request to delete the "<merchant>" membership card

    Examples:
      | merchant      | status_code_returned |state       |progress_display_text | current_value |target_value|suffix|
      | Wasabi        | 200                  |inprogress  |4/7 stamps           |  4            | 7          |stamps|
