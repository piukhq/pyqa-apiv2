...
#      | HarveyNichols | 200                  | join_failed  | fail         |
  ...
  Scenario Outline: Remove active scheme from the wallet
    ...
    Examples:
      ...
      | Iceland       | 409                  | join         | active       | Loyalty card cannot be deleted until the Join process has completed | JOIN_IN_PROGRESS |
      | Wasabi        | 409                  | join         | active       | Loyalty card cannot be deleted until the Join process has completed | JOIN_IN_PROGRESS |
      | HarveyNichols | 409                  | join         | active       | Loyalty card cannot be deleted until the Join process has completed | JOIN_IN_PROGRESS |
