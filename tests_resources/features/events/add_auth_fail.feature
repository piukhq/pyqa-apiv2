...
Feature: Verify event for add and authorise fail loyalty card
  ...
  Scenario: Verify events for add and auth field journey with unauthorised loyalty card
    ...
    When I perform POST request to add and auth "Wasabi" membership card with "unauthorised" with "202"
    Then verify that for lloyds_user data stored in after "unauthorised" journey for "Wasabi"
  ...
  Scenario: Verify event generate for fail add and auth requests in multiwallet
    ...
    When I perform POST request to add and auth "Wasabi" membership card with "unauthorised" with "202"
    Then verify that for halifax_user data stored in after "unauthorised" journey for "Wasabi"
